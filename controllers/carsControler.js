...
module.exports.deleteCar = async (req, res) => {
  console.log("Request to delete car with ID:", req.params.id);
  try {
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(400).json({ message: "Invalid car ID format" });
    }
    const carOnLocation = await Location.findOne({ carId: id });
    if (carOnLocation) {
      return res.status(400).json({ message: "Car cannot be deleted because it is associated with a location" });
    }
    const deletedCar = await Cars.findByIdAndDelete(id);
    
    return res.status(200).json({ message: "Car deleted successfully", deletedCar });
  } catch (error) {
